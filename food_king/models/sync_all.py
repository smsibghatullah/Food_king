                                                            ...
                                                            if product_id not in printed_ids:
                                                                ...
                                                                if product_id == posid['item_id']:
                                                                    line_vals.append((0, 0, {
                                                                        ...
                                                                        'qty': posid['quantity'],
                                                                        'price_unit': float(price)  + float(line_topping_ids_price) + float(item_variation_currency_total) if product_tax.price_include and product_tax else (float(price)  + float(line_topping_ids_price) + float(item_variation_currency_total))/((100+float(product_tax.amount))/100) if product_tax else 0 ,
                                ...
                                if customer_ids:
                                    ...
                                    if config_id:
                                        if search_pos_session:
                                            ...
                                            sound = AudioSegment.from_file("/home/muhammad/project/odoo17/custom_addons_food_king/food_king/static/src/sounds/bell.wav")
                                            play(sound)
                                ...
                                if data_filter_by_branch == pos_data1['branch_id']:
                                    ...
                                    if customer_ids:
                                        ...
                                        line_vals.append((0, 0, {
                                            ...
                                            'qty': 1,
                                            'company_id': self.company_id.id or Foodking_Ids.company_id.id,
                                            'price_unit': float(delivery_charge_currency_price) if delivery_charges.taxes_id and delivery_charges.taxes_id.price_include  else float(delivery_charge_currency_price)/((100+delivery_charges.taxes_id.amount )/100) if delivery_charges.taxes_id else 0 ,
